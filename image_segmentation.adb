...
package body Image_Segmentation is
   ...
   begin
      if Img1'First(1) /= Img2'First(1) or else Img1'Last(1) /= Img2'Last(1) or else
         Img1'First(2) /= Img2'First(2) or else Img1'Last(2) /= Img2'Last(2) then
   ...
   procedure Global_Threshold
     (Input     : in  Image_Grid;
      ...
      Threshold : in  Pixel_Intensity) is
   begin
   ...
   procedure Otsu_Threshold
     (Input            : in  Image_Grid;
      ...
      Total_Pixels : constant Float := Float(Input'Length(1) * Input'Length(2));
      ...
      for T in Pixel_Intensity loop
         ...
         if Weight_B > 0.0 then
            ...
            declare
               ...
               Mean_F : constant Float := (Sum_Total - Sum_B) / Weight_F;
               Between_Var : constant Float := Weight_B * Weight_F * ((Mean_B - Mean_F) ** 2);
            begin
   ...
   begin
      ...
      -- Initialize centroids evenly across intensity spectrum
      for I in 1 .. K loop
      ...
      for Iter in 1 .. Max_Iter loop
            ...
            for C in Input'Range(2) loop
               ...
               begin
                  for K_Idx in 1 .. K loop
                     declare
                        Dist : constant Float := Abs (Val - Centroids(K_Idx));
                     begin
                  ...
                  Sums(Natural(Best_K)) := Sums(Natural(Best_K)) + Long_Integer(Input(R, C));
                  Counts(Natural(Best_K)) := Counts(Natural(Best_K)) + 1;
               end;
            end loop;
         end loop;
         ...
         for K_Idx in 1 .. K loop
            if Counts(K_Idx) > 0 then
               Centroids(K_Idx) := Float(Sums(K_Idx)) / Float(Counts(K_Idx));
            end if;
         end loop;
      ...
      for I in 1 .. K loop
         Centroids_Out(Centroids_Out'First + I - 1) := Pixel_Intensity(Natural(Centroids(I)));
      end loop;
   ...
   procedure Region_Growing
     (Input        : in  Image_Grid;
      ...
      Connectivity : in  Connectivity_Mode := Four_Connected) is
      ...
      Dx_8 : constant array (1 .. 8) of Integer := (-1, 1, 0, 0, -1, -1, 1, 1);
      Dy_8 : constant array (1 .. 8) of Integer := (0, 0, -1, 1, -1, 1, -1, 1);

      Num_Neighbors : constant Positive := (if Connectivity = Four_Connected then 4 else 8);
   begin
      -- Initialize labels to 0 (unsegmented)
      ...
      for Seed of Seeds loop
         if Seed.Row not in Input'Range(1) or else Seed.Col not in Input'Range(2) then
         ...
         if Labels(Seed.Row, Seed.Col) = 0 then
            ...
            declare
               Seed_Val : constant Pixel_Intensity := Input(Seed.Row, Seed.Col);
            begin
               while not Queue.Is_Empty loop
                  ...
                  begin
                     ...
                     for N in 1 .. Num_Neighbors loop
                        declare
                           NR : constant Integer := Integer(P.Row) + (if Connectivity = Four_Connected then Dx_4(N) else Dx_8(N));
                           NC : constant Integer := Integer(P.Col) + (if Connectivity = Four_Connected then Dy_4(N) else Dy_8(N));
                        begin
                           if NR in Integer(Input'First(1)) .. Integer(Input'Last(1)) and then
                              NC in Integer(Input'First(2)) .. Integer(Input'Last(2))
                           then
                              declare
                              ...
                              begin
                                 if Labels(R_Dim, C_Dim) = 0 then
                                    if Abs (Integer(Input(R_Dim, C_Dim)) - Integer(Seed_Val)) <= Integer(Tolerance) then
                                       Labels(R_Dim, C_Dim) := Current_Label;
                                       Queue.Append ((Row => R_Dim, Col => C_Dim));
                                    end if;
                                 end if;
                              end;
                           end if;
   ...
   procedure Edge_Based_Segmentation
     (Input     : in  Image_Grid;
      ...
      Threshold : in  Pixel_Intensity    := 50) is
      ...
      Sobel_X : constant Kernel_Matrix := ((-1, 0, 1), (-2, 0, 2), (-1, 0, 1));
      Sobel_Y : constant Kernel_Matrix := ((-1, -2, -1), ( 0,  0,  0), ( 1,  2,  1));

      Prewitt_X : constant Kernel_Matrix := ((-1, 0, 1), (-1, 0, 1), (-1, 0, 1));
      Prewitt_Y : constant Kernel_Matrix := ((-1, -1, -1), ( 0,  0,  0), ( 1,  1,  1));

      K_X : Kernel_Matrix := (if Op = Sobel then Sobel_X else Prewitt_X);
      K_Y : Kernel_Matrix := (if Op = Sobel then Sobel_Y else Prewitt_Y);
   begin
      Validate_Matching_Dimensions (Input, Output);

      -- Default borders to 0
      ...
      for R in Input'First(1) + 1 .. Input'Last(1) - 1 loop
         for C in Input'First(2) + 1 .. Input'Last(2) - 1 loop
            ...
            for DR in -1 .. 1 loop
               for DC in -1 .. 1 loop
                  declare
                     Val : constant Integer := Integer(Input(Dimension(Integer(R) + DR), Dimension(Integer(C) + DC)));
                  begin
                     Gx := Gx + Val * K_X(DR, DC);
                     Gy := Gy + Val * K_Y(DR, DC);
                  end;
   ...
   procedure Watershed_Segmentation
     (Input  : in  Image_Grid;
      ...
      Current_Label : Pixel_Label := 1;
   begin
      -- Simple simulated morphological watershed via threshold flooding
      for R in Labels'Range(1) loop
         for C in Labels'Range(2) loop
            Labels(R, C) := 0;
         end loop;
